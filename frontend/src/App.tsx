...
function ConsultationsPage({ navigate }: { navigate: Navigate }) {
  ...
  return <ProductShell navigate={navigate} section="consultations"><main className="page-shell history-page"><p className="eyebrow">Patient consult records</p><h1>Consultations</h1><p className="page-intro">Completed specialty-care consultations from this Lamina workspace. No referral has been submitted.</p>
...
function ProfilePage({ navigate }: { navigate: Navigate }) {
  return <ProductShell navigate={navigate} section="profile"><main className="page-shell profile-page"><button className="text-button back-link" onClick={() => navigate('/')}>← Landing</button><div className="agent-panel"><span className="clinician-avatar">LS</span><p className="eyebrow">Synthetic clinician profile</p><h1>{PCP_NAME}</h1><p>Primary Care · Oakland, CA</p><p>{PCP_AGENT_NAME}: <strong>Active</strong></p><p className="muted-note">This demo does not include account authentication or production practice verification.</p><button className="button-primary" onClick={() => navigate('/agent')}>View My Agent →</button></div></main></ProductShell>
}
