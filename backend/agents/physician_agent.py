...
def evaluate_physician(
    ...
    for case, overlap in matches:
        evidence.append(Evidence(kind=EvidenceKind.PRACTICE_SIMILARITY, detail=f"Synthetic case mix includes {case.label}; overlapping signals: {', '.join(overlap)}. This is a fit signal, not a quality measure."))
    evidence.append(Evidence(kind=EvidenceKind.OPERATIONAL, detail=f"Approximate next availability: {profile.availability_days} days; {_insurance_status(patient, profile)}"))
    ...
    if profile.id == "physician-jung":
        ...
        evidence.insert(0, Evidence(kind=EvidenceKind.EXPLICIT_RULE, detail=profile.explicit_rules[0]))
